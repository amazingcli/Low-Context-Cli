...
test('a turn retrieves verified source into context and records it on disk', async () => {
    await withTempHome(async () => {
        ...
        const agent = new Agent({
            workspace,
            provider,
            model: (await provider.listModels())[0],
        ...
        const messages = await workspace.stores.conversations.messages(conversations[0].id);
...
test('context is capped for a model with a small window', async () => {
    await withTempHome(async () => {
        ...
        const agent = new Agent({
            workspace,
            provider,
            model: { ...(await provider.listModels())[0], context_limit: 4_000 },
            registry: registryFromConfig(workspace.config),
            logger: new NullLogger(),
            events: new AgentEventLog(),
            ui: silentUi(),
