...
function item(kind, content, priority = 0.5) {
...
test('items that do not fit are reported as dropped, never silently omitted', () => {
    ...
    for (let i = 0; i < 10; i += 1)
        builder.add(item('code', `file ${i} `.repeat(60)));
    const selected = builder.build();
    const report = builder.report(selected);
...
test('deep strategy admits more code than minimal, but still bounded', () => {
    const make = (strategy) => {
        ...
        for (let i = 0; i < 40; i += 1)
            builder.add(item('code', `export function fn${i}() { return ${i}; }`.repeat(20)));
        const selected = builder.build();
...
test('extractive summarisation preserves decisions and outcomes', () => {
    const messages = [
        { id: 'm1', conversation_id: 'c', timestamp: '', role: 'user', content: 'We decided to use the existing OAuth implementation rather than writing a new one.' },
        { id: 'm2', conversation_id: 'c', timestamp: '', role: 'assistant', content: 'Understood. I will wire the existing provider into the login route.' },
        { id: 'm3', conversation_id: 'c', timestamp: '', role: 'assistant', content: 'The test run passed after the fix.' },
        { id: 'm4', conversation_id: 'c', timestamp: '', role: 'user', content: 'The staging API base url is /v2.' },
    ];
    const summary = extractiveSummarize(messages);
    assert.match(summary.summary, /Decisions/);
    assert.equal(summary.decisions.length, 1);
    assert.match(summary.decisions[0], /OAuth implementation/);
...
test('compaction writes memory records with source references and keeps the transcript', async () => {
    ...
    const appended = [];
    for (let i = 0; i < 6; i += 1) {
        appended.push(await conversations.append(conversation.id, {
    ...
    for (const ref of withRefs[0]?.source_refs ?? []) {
        assert.ok((await conversations.messagesByIds([ref.message_id])).length === 1);
    }
