...
test('analysis extracts symbols, imports and tests without noise', () => {
    const analysis = analyzeSource('src/auth/service.ts', TREE['src/auth/service.ts']);
    ...
    const testAnalysis = analyzeSource('src/payments/receipt.test.ts', TREE['src/payments/receipt.test.ts']);
...
test('incremental refresh re-analyses only changed files', async () => {
    ...
    await writeFile(join(root, 'src/payments/receipt.ts'), 'export function parseReceipt(input: string): number {\n  return Number(input.replace(/[^0-9]/g, "")) * 2;\n}\n', 'utf8');
...
test('modules group files and project map renders a navigation tree', async () => {
    ...
    const entry = renderFileMapEntry(files.find((file) => file.path === 'src/auth/service.ts'));
