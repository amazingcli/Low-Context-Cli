...
function trustedConfig() {
    ...
    return { ...base, permissions: { ...base.permissions, mode: 'trusted' } };
}
async function toolContext(root, overrides = {}) {
...
test('edit_file requires an exact, unique match and verifies the write', async () => {
    ...
    const ok = await registry.invoke(newToolCall('edit_file', { path: 'src/a.ts', old_string: 'return 1;', new_string: 'return 2;' }), ctx);
    ...
    const ambiguous = await registry.invoke(newToolCall('edit_file', { path: 'src/b.ts', old_string: '= 1;', new_string: '= 2;' }), await toolContext(ambiguousRoot));
    assert.equal(ambiguous.result.ok, false);
    assert.match(ambiguous.result.summary, /occurs 2 times/);
    const replaced = await registry.invoke(newToolCall('edit_file', { path: 'src/b.ts', old_string: '= 1;', new_string: '= 2;', replace_all: true }), await toolContext(ambiguousRoot));
...
test('run_command sends huge output to an artifact and keeps the diagnostics inline', async () => {
    ...
    const ctx = await toolContext(root);
    const registry = registryFromConfig(ctx.config);
    const result = await registry.invoke(newToolCall('run_command', { command: 'for i in $(seq 1 5000); do echo "line $i of the build log"; done; echo "error TS1234: boom"' }), ctx);
...
test('OutputManager spills large output and extracts error lines', async () => {
    ...
    assert.equal(await readFile(processed.artifactPath, 'utf8'), body);
});
