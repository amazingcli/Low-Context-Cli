...
test('gateway refusals are recognised, including the two that degrade gracefully', () => {
    ...
    const overflow = new Error('HTTP 400: {"error":{"message":"This endpoint\'s maximum context length is 32768 tokens. However, you requested about 36721 tokens"}}');
...
test('a shrunk budget leaves room for a retry after an overflow rejection', () => {
    ...
    const tokens = (items) => items.reduce((sum, item) => sum + item.tokens, 0);
