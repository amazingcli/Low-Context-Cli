...
export class FileTaskStore {
    ...
    async upsert(state) {
        ...
        const updated = { ...state, updated_at: nowIso() };
    ...
    async get(id) {
        ...
        if (cached)
            return cached;
        const state = await readJsonIfExists(this.path(id));
        if (state)
    ...
    async list(input = {}) {
        ...
        for (const name of entries) {
            ...
            if (!state)
                continue;
            if (input.project_id && state.project_id !== input.project_id)
                continue;
            out.push(state);
        }
...
export function renderTaskState(state) {
    ...
    if (state.goal && state.goal !== state.title)
    ...
    if (state.completed.length > 0)
        lines.push(`Done: ${state.completed.slice(-5).join('; ')}`);
    if (state.relevant_files.length > 0)
        lines.push(`Files: ${state.relevant_files.slice(0, 10).join(', ')}`);
    if (state.relevant_symbols.length > 0)
        lines.push(`Symbols: ${state.relevant_symbols.slice(0, 12).join(', ')}`);
    if (state.open_questions.length > 0)
        lines.push(`Open questions: ${state.open_questions.join('; ')}`);
    return lines.join('\n');
}
