...
export class FileSessionStore {
    ...
    async get(id) {
        ...
        if (cached)
            return cached;
        const session = await readJsonIfExists(this.path(id));
        if (session) {
            if (!session.status)
                session.status = 'idle';
    ...
    async list(input = {}) {
        ...
        for (const name of entries) {
            ...
            if (!session)
                continue;
            if (input.project_id && session.project_id !== input.project_id)
                continue;
    ...
    async update(id, patch) {
        ...
        if (!session)
            return undefined;
        const merged = { ...session, ...patch, id: session.id, updated_at: nowIso() };
        await this.save(merged);
        return merged;
    }
    async saveTaskState(id, state) {
        const session = await this.get(id);
        if (!session)
            return;
