...
export class ProviderHttp {
    ...
    mapFetchError(error, aborted) {
        if (aborted) {
            return new LowContextError('CANCELLED', 'Request was cancelled');
        }
        return wrapError('PROVIDER_HTTP', 'Failed to reach the provider API', error);
    }
}
/**
 ...
 */
export function isToolCallingUnsupported(error) {
    const message = error instanceof Error ? error.message : String(error);
    return (/no endpoints? found that support tool/i.test(message) ||
...
export function isContextOverflow(error) {
    const message = error instanceof Error ? error.message : String(error);
    return (/maximum context length/i.test(message) ||
...
export function parseContextLimit(error) {
    const message = error instanceof Error ? error.message : String(error);
    const patterns = [
