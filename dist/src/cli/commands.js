...
import { defaultConfig, globalConfigPath, mergeConfig, projectConfigPath, saveGlobalConfig, validateConfig, } from '../core/config.js';
import { listLogFiles, pruneLogs, readLogTail } from '../core/logger.js';
...
const VERSION = '0.1.0';
/* ================================= helpers ================================= */
async function withWorkspace(ctx, options = {}) {
    return ctx.openWorkspace(options);
}
function describeIndexSources(ctx) {
    return ctx.configSources.length === 0 ? '(defaults only)' : ctx.configSources.join(', ');
}
function requireOk(problems, ui) {
...
/* ================================== init =================================== */
async function providersCommand(ctx, sub) {
    const { ui, config } = ctx;
    switch (sub) {
        case 'list':
        case '': {
        ...
        case 'add': {
            ...
            const kind = (flagString(ctx.args.flags, 'kind') ?? 'openai');
            if (!name) {
                ...
                if (process.stdin.isTTY === true)
                    return providersAddInteractive(ctx);
        ...
        case 'remove': {
            ...
            if (next.active_provider === name)
                next.active_provider = undefined;
            await saveGlobalConfig(next);
...
async function modelsCommand(ctx, sub) {
    const { ui, config } = ctx;
    switch (sub) {
        case 'list':
        case '': {
...
/* ================================== index ================================== */
async function indexCommand(ctx, sub) {
    const { ui } = ctx;
    ...
    if (effective === 'status') {
        ...
        if (stale.length > 0)
            ui.warn(`The index is behind the working tree. Run \`lc index refresh\` before relying on it.`);
        return 0;
    }
...
/* =================================== map =================================== */
async function mapCommand(ctx, sub) {
    const { ui } = ctx;
    const workspace = await withWorkspace(ctx);
...
async function filesCommand(ctx, sub) {
    const { ui } = ctx;
    const workspace = await withWorkspace(ctx);
    const files = await workspace.stores.index.listFiles(workspace.project.id);
    if (sub === 'show') {
...
async function searchCommand(ctx) {
    ...
    ui.table(['score', 'path', 'state', 'matched'], fileHits.map((hit) => [hit.score.toFixed(2), hit.file.path, hit.file.verification_state, hit.matched.join(', ')]));
    ...
    if (memoryHits.length > 0) {
        ...
        ui.table(['score', 'id', 'type', 'summary'], memoryHits.map((hit) => [hit.score.toFixed(2), hit.record.id, hit.record.type, truncate(hit.record.summary, 70)]));
    }
...
/* ================================= memory ================================== */
async function memoryCommand(ctx, sub) {
    const { ui, config } = ctx;
    const workspace = await withWorkspace(ctx);
    ...
    switch (effective) {
        case 'list': {
            ...
            ui.table(['id', 'type', 'scope', 'importance', 'updated', 'summary'], records.map((record) => [
                record.id,
                record.type,
                record.scope,
                record.importance,
        ...
        case 'show': {
            ...
            if (flagBool(ctx.args.flags, 'history')) {
                ...
                if (chain.length > 0 || newer) {
                    ...
                    if (newer)
                        ui.out(`  → ${newer.id}  ${newer.status}  ${truncate(newer.summary, 80)}`);
                }
                else {
                    ui.out('');
        ...
        case 'supersede': {
            ...
            if (replacement)
                ui.out(`  ${truncate(replacement.summary, 100)}`);
            return 0;
        }
        case 'delete': {
            const id = ctx.args.positionals[0];
        ...
        case 'forget': {
            ...
            for (const hit of hits)
                ui.out(`  ${hit.record.id}  ${truncate(hit.record.summary, 80)}`);
            ...
            let deleted = 0;
            for (const hit of hits)
                if (await store.delete(hit.record.id))
                    deleted += 1;
...
function renderMemoryRecord(ui, record) {
...
/* ================================= sessions ================================ */
async function sessionsCommand(ctx, sub) {
    const { ui } = ctx;
    const workspace = await withWorkspace(ctx);
    ...
    switch (sub) {
        ...
        case '': {
            ...
            ui.heading('Sessions');
            ui.table(['id', 'status', 'model', 'messages', 'updated', 'title'], sessions.map((session) => [
                session.id,
                session.status,
                session.model ?? '-',
...
async function configCommand(ctx, sub) {
    const { ui } = ctx;
    ...
    switch (effective) {
        ...
        case 'set': {
            ...
            if (problems.length > 0 && !requireOk(problems, ui))
                return 2;
...
function buildPatch(key, raw) {
    const parts = key.split('.');
    let value = /^-?\d+$/.test(raw) ? Number(raw) : raw === 'true' ? true : raw === 'false' ? false : raw;
...
/* ============================== providers add ============================= */
const PROVIDER_KINDS = ['openai', 'anthropic', 'gemini', 'custom', 'local', 'mock'];
const KIND_BASE_URLS = {
 ...
 */
async function providersAddInteractive(ctx) {
    const { ui, config } = ctx;
    ...
    const kind = PROVIDER_KINDS[Number.isFinite(kindChoice) ? Math.max(0, Math.min(PROVIDER_KINDS.length - 1, kindChoice - 1)) : 0];
    ...
    if (storeKey && apiKey === '')
        ui.warn('No key entered; the environment variable will be used instead.');
    ui.out('');
    ...
    if (apiKey)
        await setCredential(`providers.${id}.api_key`, apiKey);
    ui.success(`Added provider "${id}" (${kind}) with model ${modelId}.`);
    if (!config.active_provider)
        ui.out(`Activate it with: lc models use ${modelId} --provider ${id}`);
    return 0;
}
/* =============================== permissions ============================== */
async function permissionsCommand(ctx, sub) {
    const { ui, config } = ctx;
    const effective = sub === '' ? 'show' : sub;
    switch (effective) {
        case 'show': {
        ...
        case 'mode': {
            const mode = ctx.args.positionals[0];
...
/* ================================== tools ================================= */
async function toolsCommand(ctx, sub) {
    const { ui, config } = ctx;
    ...
    ui.heading('Tools');
    ui.table(['name', 'category', 'mutating', 'enabled', 'purpose'], registry.list().map((tool) => [tool.name, tool.category, tool.mutating ? 'yes' : 'no', 'yes', truncate(tool.description, 60)]));
    return 0;
}
/* ================================== logs ================================== */
async function logsCommand(ctx, sub) {
    const { ui } = ctx;
    ...
    switch (effective) {
        case 'list': {
            ...
            if (files.length === 0)
                ui.info('No logs yet. Run a request first.');
            return 0;
        }
        case 'path':
            ui.result(dir);
...
async function doctorCommand(ctx) {
    ...
    if (!provider) {
        ...
        checks.push(!needsSecret || secret
    ...
    try {
        ...
        try {
            ...
            checks.push({ name: 'Filesystem', status: 'FAIL', detail: `cannot write to ${workspace.paths.dir}: ${error.message}` });
        }
        try {
            ...
            checks.push(outcome.exit_code === 0
            ...
            checks.push({ name: 'Terminal', status: 'FAIL', detail: error.message, fix: 'Ensure /bin/sh (or cmd.exe) is available' });
        }
        if (config.embedding.enabled) {
            try {
                const embedder = await createEmbeddingClient(config);
                checks.push(embedder
                ...
                checks.push({ name: 'Embeddings', status: 'WARN', detail: error.message, fix: 'Set embedding.enabled=false to use keyword retrieval' });
            }
        }
        else {
    ...
    for (const check of checks) {
        ...
        if (check.fix)
            ui.result(`      ${ui.dim(`fix: ${check.fix}`)}`);
...
/* ================================= context ================================ */
async function contextCommand(ctx, sub) {
    const { ui } = ctx;
    const workspace = await withWorkspace(ctx);
    ...
    if (sub === 'explain' || flagBool(ctx.args.flags, 'explain')) {
        ...
        ui.table(['score', 'verified', 'source', 'reason'], result.trace.candidates.slice(0, 20).map((candidate) => [
    ...
    if (dry.trace !== undefined) {
        ...
        for (const step of dry.trace.steps)
            ui.out(`  ${step.stage.padEnd(16)} ${step.kept}/${step.candidates} kept  ${ui.dim(step.detail)}`);
    }
    return 0;
}
/* ================================= analyze =============================== */
async function analyzeCommand(ctx) {
    const { ui } = ctx;
    ...
    ui.table(['module', 'files', 'languages', 'key symbols'], modules.slice(0, 30).map((module) => [module.path, String(module.file_count), module.languages.join(','), truncate(module.top_symbols.slice(0, 4).join(', '), 40)]));
