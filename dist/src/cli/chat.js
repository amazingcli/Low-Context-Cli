    ...
    try {
        for (;;) {
            ...
            if (line.startsWith('/')) {
                ...
                if (handled.taskState)
                    taskState = handled.taskState;
                if (handled.endSession)
                    break;
                continue;
            }
...
async function handleSlashCommand(ctx, workspace, agent, line, state) {
    const { ui } = ctx;
...
function printBanner(ctx, workspace, model, sessionId) {
    const { ui } = ctx;
    ui.out('');
