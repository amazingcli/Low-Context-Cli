    ...
    return {
        ...
        toolResult: (result) => {
            ...
            if (!result.ok && result.error && ui.verbose)
                ui.out(ui.dim(`        ${truncate(result.error, 150)}`));
...
function renderContextBar(used, usable, width = 12) {
 ...
 */
function printTurnSummary(ctx, info) {
    const { ui } = ctx;
    if (ui.quiet)
        return;
