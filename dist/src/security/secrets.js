...
export async function saveCredentials(map) {
...
export async function setCredential(ref, value) {
...
export async function deleteCredential(ref) {
    const map = await loadCredentials();
    if (!(ref in map))
        return false;
    delete map[ref];
 ...
 */
export async function resolveSecret(ref, envNames = []) {
 ...
 * into model context.
 */
export const SENSITIVE_ENV_PATTERN = /(API[_-]?KEY|SECRET|TOKEN|PASSWORD|PASSWD|PRIVATE[_-]?KEY|CREDENTIAL|SESSION[_-]?ID|AUTH|COOKIE|ACCESS[_-]?KEY|CLIENT[_-]?SECRET)/i;
