...
export function isReadOnlyCommand(command) {
    ...
    if (/[;&|><]/.test(trimmed.replace(/\|\|\s*(true|exit 0)$/, '')))
        return false;
...
export function commandRisk(command) {
    for (const pattern of CATASTROPHIC_COMMAND_PATTERNS) {
        if (pattern.re.test(command))
            return { dangerous: true, why: pattern.why };
    }
    return { dangerous: false };
}
/** True when the command can change the working tree in a meaningful way. */
export function commandIsMutating(command) {
...
export class PermissionEngine {
     ...
     */
    resolvePath(rawPath, projectRoot, options = {}) {
...
function matches(rule, request, resource) {
    if (rule.target !== 'any' && rule.target !== request.resource)
        return false;
