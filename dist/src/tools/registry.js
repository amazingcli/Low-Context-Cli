...
export class ToolRegistry {
    ...
    constructor(policy, options = {}) {
        for (const tool of options.tools ?? defaultTools())
    ...
    isEnabled(name) {
        if (this.disabled.has(name))
            return false;
        if (this.enabled.size > 0 && !this.enabled.has(name))
            return false;
        return true;
    }
    ...
    preview(call, ctx) {
        ...
        if (!tool)
            return { allowed: false, prompt: false, reason: `unknown tool: ${call.name}` };
    ...
    async invoke(call, ctx) {
        ...
        const verdict = this.engine.decide(request);
        if (!verdict.allowed) {
            const result = toolResultFrom(call, { ok: false, summary: `Refused: ${verdict.reason}.`, error: verdict.reason }, { bytes: 0, truncated: false, duration_ms: Date.now() - started });
        ...
        if (verdict.prompt) {
            ...
            if (!approved) {
                const result = toolResultFrom(call, { ok: false, summary: `Not approved: ${request.summary}`, error: 'not approved by user' }, { bytes: 0, truncated: false, duration_ms: Date.now() - started });
        ...
        try {
            ...
            const message = error instanceof ToolArgumentError ? error.message : error.message;
...
function requestFor(tool, call, ctx) {
    if (tool.permission) {
        try {
...
export function registryFromConfig(config, options = {}) {
