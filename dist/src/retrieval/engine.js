...
export async function runRetrieval(deps, options) {
    ...
    const note = (step, detail, candidateCount, kept, durationMs) => {
    ...
    const modules = await safe(deps.indexStore.listModules(deps.projectId ?? ''), []);
    ...
    for (const match of symbolMatches) {
        ...
        if (existing)
            existing.lexical = (existing.lexical ?? 0) + match.score;
        else {
    ...
    let conversationInputs = [];
    if (trace.intent.historical || trace.intent.kind === 'memory_query') {
        const convStarted = Date.now();
        const convHits = await safe(deps.conversationStore.searchMessages({
            text: options.query,
            project_id: deps.projectId,
            limit: 16,
        }), []);
    ...
    const recentInputs = (deps.recentUserMessages ?? []).slice(-6).map((m) => ({
    ...
    const dependencyManaged = new Set();
    const depInputs = [];
    ...
    for (const pair of keyed) {
        ...
        if (!existing || pair.item.score > existing.score)
            byKey.set(pair.key, pair.item);
    }
    ...
    if (!options.skipVerification && deps.projectRoot) {
        ...
        for (const item of topFiles) {
            ...
            if (!isInside(deps.projectRoot, abs))
                continue;
        ...
        note('verify', 'read candidate files from disk', topFiles.length, topFiles.filter((i) => i.content).length, Date.now() - verifyStarted);
    }
    let candidates = deduped.kept.map((item) => {
        const meta = indexMeta.get(item.id) ?? memoryMeta.get(item.id) ?? gitMeta.get(item.id);
        return {
            id: item.id,
            source: 'index_lexical',
    ...
    return {
        intent: trace.intent,
        candidates,
        verifiedFiles: verifiedFiles.map((c) => ({ id: c.id, path: c.file_path ?? '', verification_state: c.verification_state ?? 'verified' })),
...
/* --------------------------------- helpers -------------------------------- */
function exactBonus(query, haystacks) {
    const terms = analyzeQuery(query);
    let hits = 0;
    for (const term of terms.raw) {
        if (haystacks.some((h) => h.toLowerCase().split(/[./\\]/).some((part) => part === term)))
            hits += 1;
    }
...
function fileReason(file) {
    const parts = [];
    if (file.summary)
        parts.push('index summary match');
...
export function candidatesToContextItems(candidates, _options) {
    const out = [];
