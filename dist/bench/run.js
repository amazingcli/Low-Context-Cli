...
const metrics = [];
function record(bench, metric, value, unit, note) {
 ...
 */
async function makeRepo(root, fileCount, filesPerModule = 12) {
    const paths = [];
...
/* ------------------------------ benchmark parts ---------------------------- */
async function benchIndexing(home) {
    const sizes = [
    ...
    for (const size of sizes) {
        ...
        console.log(`   ${size.label.padEnd(11)} ${String(size.files).padStart(5)}   ` +
        ...
        check(`incremental indexing touches one file (${size.label})`, changed <= 1, `${changed} file(s) re-analysed after a single edit`);
        check(`incremental is faster than cold (${size.label})`, incremental.ms <= cold.ms, `${incremental.ms.toFixed(0)}ms vs ${cold.ms.toFixed(0)}ms cold`);
        check(`index summary present (${size.label})`, (summary?.files ?? 0) === size.files, `${summary?.files ?? 0} files indexed`);
        await workspace.flush();
    }
}
async function benchRetrieval(home) {
    console.log('\n## Retrieval');
    ...
    for (const query of queries) {
        const run = await time(() => runRetrieval({
            config,
        ...
        const tokens = run.value.candidates.reduce((sum, candidate) => sum + estimateTokens(candidate.content ?? candidate.excerpt ?? candidate.reason).tokens, 0);
        console.log(`   ${query.slice(0, 33).padEnd(34)} ${run.ms.toFixed(0).padStart(5)}   ` +
...
async function benchMemory(home) {
    console.log('\n## Memory');
    ...
    for (const target of targets) {
        ...
        for (let i = written; i < target; i += 1) {
            batch.push(workspace.stores.memory.write({
                type: ['FACT', 'DECISION', 'BUG', 'FIX', 'PROJECT_KNOWLEDGE'][i % 5],
                scope: ['global', 'project', 'module', 'session', 'task'][i % 5],
        ...
        console.log(`   ${String(target).padStart(7)}   ${writeMs.toFixed(0).padStart(10)}   ${avg.toFixed(2).padStart(11)}   ${p95.toFixed(2).padStart(14)}`);
...
async function benchContext(home) {
    console.log('\n## Context');
    ...
    const compact = await time(() => compactMessages({
        messages: history,
    ...
    const extraction = await time(async () => candidatesFromUserMessage('Remember: the receipts API uses PostgreSQL and validates amounts in cents.', {
        projectId: workspace.project.id,
    }));
...
async function benchLargeOutput(home) {
    console.log('\n## Large tool output');
    ...
    for (let i = 0; i < 200_000; i += 1) {
        if (i % 20_000 === 0)
            lines.push(`src/file_${i}.ts(${i},3): error TS2304: Cannot find name 'value_${i}'.`);
        else
            lines.push(`[build] compiling module ${i} ... ok in ${i % 97}ms`);
    }
    ...
    const slice = await time(async () => {
        const full = await readFile(processed.value.artifactPath, 'utf8');
        return full.split('\n').filter((line) => line.includes('error TS')).length;
    });
    record('output', 'artifact_grep_latency', slice.ms, 'ms');
    check('artifact is retrievable on demand', slice.value >= 10, `${slice.value} diagnostics read back in ${slice.ms.toFixed(0)}ms`);
}
/* ----------------------------------- main --------------------------------- */
async function main() {
    const started = performance.now();
