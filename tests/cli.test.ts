...
function captureFetch(payload: unknown): { calls: Captured[]; restore: () => void } {
...
test('Gemini authenticates with x-goog-api-key and never sends a Bearer token', async () => {
  ...
  try {
    ...
    const call = capture.calls[0] as Captured;
...
test('the permission bypass removes prompts but never unlocks catastrophic commands', () => {
  ...
  const write = { summary: 'write file', resource: 'path' as const, subject: 'src/a.ts', destructive: true };
  const bomb = { summary: 'wipe root', resource: 'command' as const, subject: 'rm -rf /', destructive: true };
  const denied = { summary: 'denied path', resource: 'command' as const, subject: 'rm -rf /important', destructive: true };
