...
test('JsonlLog appends, reads by ordinal and rebuilds a stale offset file', async () => {
  ...
  const log = new JsonlLog<Row>(path, { flushEvery: 2 });
  ...
  for (let i = 0; i < 5; i += 1) await log.append({ id: `r${i}`, value: i });
  ...
  await log.flush();
  const fresh = new JsonlLog<Row>(path);
...
test('JsonlLog retain rewrites the log and keeps offsets consistent', async () => {
  const dir = await tempDir();
  const log = new JsonlLog<Row>(join(dir, 'rows.jsonl'));
  ...
  const reloaded = new JsonlLog<Row>(join(dir, 'rows.jsonl'));
...
test('InvertedIndex survives a serialise/deserialise round trip', () => {
  ...
  const restored = InvertedIndex.deserialize(JSON.parse(JSON.stringify(index.serialize())) as ReturnType<InvertedIndex['serialize']>);
...
test('PersistentLexicalIndex persists through its debounced flush without recursing', async () => {
  ...
  const raw = JSON.parse(await readFile(path, 'utf8')) as { doc_count: number };
...
test('LocalVectorStore returns exact nearest neighbours and survives reload', async () => {
  ...
  await store.upsert([
    { id: 'auth', vector: vectors[0] as Float32Array },
    { id: 'pay', vector: vectors[1] as Float32Array },
    { id: 'doc', vector: vectors[2] as Float32Array },
  ]);
  await store.flush();

  const query = (await embedder.embed(['token authentication']))[0] as Float32Array;
...
test('HashingEmbedder is deterministic and L2-normalised', async () => {
  ...
  assert.deepEqual(Array.from(first as Float32Array), Array.from(second as Float32Array));
  const norm = Math.sqrt(Array.from(first as Float32Array).reduce((sum, value) => sum + value * value, 0));
...
test('redaction masks credentials without destroying structure', () => {
  ...
  const nested = redact({ authorization: 'Bearer abcdefghijklmnopqrstuvwxyz', keep: 'yes' }) as Record<string, string>;
...
test('buildChildEnv drops credential-like variables by default', () => {
  const env = buildChildEnv(
    { PATH: '/usr/bin', OPENAI_API_KEY: 'sk-secret', ANTHROPIC_API_KEY: 'sk-other', HOME: '/home/x' } as NodeJS.ProcessEnv,
    {},
  );
  ...
  const allowListed = buildChildEnv({ PATH: '/usr/bin', MY_TOKEN: 'x' } as NodeJS.ProcessEnv, { allow: ['PATH', 'MY_TOKEN'] });
