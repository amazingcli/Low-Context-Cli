...
test('a file too large to send keeps its head and tail, and says what was dropped', () => {
  ...
  assert.match(parts[0] as string, /^1: line number 1 /);
  assert.match(rendered, /line\(s\) omitted: \d+–\d+ of 400/);
  assert.match(parts[parts.length - 1] as string, /^400: line number 400 /);
...
test('historical questions search conversation history, not only recent context', async () => {
  ...
  const output = await runRetrieval(
    { config: defaultConfig(), indexStore, memoryStore, conversationStore, projectRoot: root, projectId },
    { query: 'what database did we decide on for the audit log earlier', trace: true },
  );
