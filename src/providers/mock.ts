/**
 * Mock provider: a fully offline, deterministic chat provider.
 *
 * It exists so Low Context is *usable* with no network and no keys — doctor,
 * tests, and first-run demos all exercise the real agent loop through it. Its
 * replies are canned but structured, and it can emit tool-call events so the
 * tool-calling path can be tested end-to-end.
 *
 * It is labelled everywhere it appears; it is not a real model.
 */
import type {
  GenerationRequest,
  ModelDescriptor,
...
const SCRIPT: { match: RegExp; response: string }[] = [
...
export class MockProvider implements ChatProvider {
  ...
  async *generateWithToolCall(_request: GenerationRequest, call: ToolCall): AsyncIterable<StreamEvent> {
 ...
 */
function lastUserRequest(messages: readonly { role: string; content: string }[]): string {
