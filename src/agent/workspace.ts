...
import type { IndexRefreshResult } from '../index/indexer.js';
import { projectPaths, findProjectRoot, type ProjectPaths } from '../core/paths.js';
...
import type { ProjectStore, ConversationStore, MemoryStore, IndexStore, SessionStore, TaskStore } from '../storage/interfaces.js';
...
export interface Workspace {
  ...
  artifactsDir: string;
  /** Refresh the index if it is enabled. Safe to call repeatedly. */
  ensureIndex(options?: { force?: boolean; onProgress?: (info: { phase: string; path?: string; done: number; total: number }) => void }): Promise<IndexRefreshResult | undefined>;
  /** Paths whose content no longer matches the index. */
  stalePaths(): Promise<string[]>;
  indexSummary(): Promise<{ files: number; modules: number; symbols: number } | undefined>;
  flush(): Promise<void>;
}

export interface OpenWorkspaceOptions {
  /** Starting directory; the project root is discovered upwards when omitted. */
  root?: string;
  config: LowContextConfig;
  /**
   * Build or refresh the project index on open. Opt-in: when omitted, opening a
   * workspace is read-only and the caller decides whether to index. CLI callers
   * pass `config.index.refresh_on_start` explicitly.
   */
...
export async function openWorkspace(options: OpenWorkspaceOptions): Promise<Workspace> {
  ...
  const workspace: Workspace = {
    ...
    async ensureIndex(refreshOptions = {}) {
      if (!options.config.index.enabled) return undefined;
    ...
    async stalePaths() {
      if (!options.config.index.enabled) return [];
  ...
  if (options.refreshIndex === true) await workspace.ensureIndex({ onProgress: options.onProgress });

  return workspace;
}

/** Project ids are derived from the root path, so they survive a registry wipe. */
export function workspaceProjectId(root: string): string {
