...
import type { RetrievalOutput } from '../retrieval/engine.js';
...
import type { ChatProvider, EmbeddingSupport } from '../providers/types.js';
...
export interface AgentUi {
  ...
  contextReport?(report: ContextBudgetReport): void;
...
export interface AgentOptions {
  ...
  signal?: AbortSignal;
  embedder?: EmbeddingSupport;
  /** Run the verification pipeline after mutating tools. Defaults to config. */
  verifyAfterEdits?: boolean;
  /** Include the retrieval trace as a context item (debug mode). */
...
export interface AgentTurnResult {
  ...
  toolCalls: number;
  usage: UsageReport[];
  contextReport?: ContextBudgetReport;
...
export class Agent {
  ...
  private readonly verifyEdits: boolean;
  private conversationId: string | undefined;
  private lastContextReport: ContextBudgetReport | undefined;
  ...
  get contextReport(): ContextBudgetReport | undefined {
   ...
   */
  setUi(ui: AgentUi): void {
    this.o.ui = ui;
  }

  /** Locate the project root upwards from `cwd` when the CLI did not pass one. */
  async run(userMessage: string): Promise<AgentTurnResult> {
    ...
    while (turns < this.maxTurns) {
        ...
        for await (const event of provider.generate(request)) {
          switch (event.type) {
            ...
            case 'done':
              ...
              if (event.message.content && event.message.content.length > assistantText.length) assistantText = event.message.content;
              break;
          }
        }
      } catch (error) {
      ...
      if (streamedAny && assistantText !== '') ui.status?.('');
      if (stopReason === 'length') ui.error?.('The model reached its output limit; this response may be incomplete.');
      ...
      for (const call of calls) {
        ...
        for (const file of outcome.result.affected_files ?? []) mutatedFiles.push(file);
  ...
  private async prepareContext(query: string, conversationId?: string): Promise<PreparedContext> {
    ...
    let activeRecent: ChatMessage[] = [];
    let compactedThroughId: string | undefined;
    let compacted = false;
    if (conversationId) {
   ...
   */
  async dryRun(query: string): Promise<{ report: string; trace?: RetrievalTrace; contextBlock: string }> {
  ...
  private async ensureConversation(): Promise<{ id: string }> {
  ...
  private buildSystemInstruction(
    indexSummary: { files: number; modules: number; symbols: number } | undefined,
    memoryCount: number,
  ): string {
  ...
  private taskItem(): ContextItem | undefined {
  ...
  private async projectMapItem(): Promise<ContextItem | undefined> {
    const { workspace } = this.o;
    if (!workspace.config.index.enabled) return undefined;
  ...
  private memoryToContextItems(records: readonly MemoryRecord[]): ContextItem[] {
  ...
  private traceItem(trace: RetrievalTrace): ContextItem {
  ...
  private renderContextBlock(items: readonly ContextItem[], userMessage: string): string {
    const { workspace } = this.o;
    const config = workspace.config;
    const parts: string[] = [];
    parts.push(
    ...
    for (const item of items) {
      if (item.kind === 'retrieval_trace') continue;
      ...
      if (guarded.note) parts.push(`[low-context security: ${guarded.note}]`);
      parts.push(`### ${header}\n${guarded.content}`);
    }
    return parts.join('\n\n');
  }
  ...
  private async maybeCompact(
    ...
    conversationId: string,
    userMessage: string,
  ): Promise<{ compacted: boolean; messages: ChatMessage[]; compactedThroughId?: string }> {
    const { workspace, model, logger } = this.o;
    const config = workspace.config;
    if (!config.context.auto_compact) return { compacted: false, messages: [...recent] };
    ...
    if (older.length === 0) return { compacted: false, messages: [...recent] };

    const result = await compactMessages({
      messages: older as ChatMessage[],
      conversationId,
      conversationStore: workspace.stores.conversations,
      memoryStore: workspace.stores.memory,
      ...(this.o.sessionId === undefined ? {} : { sessionId: this.o.sessionId }),
    ...
    this.o.ui.note?.(
      `[context] compacted ${older.length} older message(s) into ${result.memoryRecords.length} memory record(s); sources kept on disk.`,
    );
  ...
  private async updateIndexAfterChange(files: readonly string[], ui: AgentUi): Promise<void> {
    const { workspace, logger, events } = this.o;
    if (!workspace.config.index.enabled) return;
    try {
      ...
      logger.warn('index.update_failed', { message: (error as Error).message });
    }
  }

  private async consolidateMemory(compactedThroughId: string | undefined, ui: AgentUi): Promise<number> {
    const { workspace, events, logger } = this.o;
    if (!workspace.config.memory.enabled || !workspace.config.memory.auto_capture) return 0;
    ...
    for (const record of extracted.written) {
      try {
        ...
        if (existing.length > 0 && existing[0] && existing[0].score > 0.85) continue;
        ...
        logger.warn('memory.write_failed', { message: (error as Error).message });
      }
    }
    if (written > 0) ui.note?.(`[memory] stored ${written} record(s) from verified actions.`);
  ...
  private toolContext() {
    ...
    return {
      ...
      conversationStore: workspace.stores.conversations,
      ...(this.o.signal === undefined ? {} : { signal: this.o.signal }),
      note: (message: string) => this.o.ui.note?.(message),
      ...(this.o.ui.confirm === undefined ? {} : { confirm: (request: PermissionRequest) => this.o.ui.confirm!(request) }),
    };
  }

  /** Exposed so the CLI can render exactly what a request would contain (§39). */
  describeBudget(): string | undefined {
...
function toProviderMessage(message: ChatMessage): ProviderMessage {
  const base: ProviderMessage = { role: message.role, content: message.content };
  if (message.tool_calls) base.tool_calls = message.tool_calls.map((call) => ({ id: call.id, name: call.name, arguments: JSON.stringify(call.arguments) }));
