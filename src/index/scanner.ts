...
import type { IndexedFile, SymbolRecord, VerificationState } from '../core/types.js';
import { newId } from '../core/ids.js';

export interface ScanOptions {
  root: string;
  /** Previous indexed file metadata: posix path -> {hash, size, mtime_ms}. */
  previous?: Map<string, { hash: string; size: number; mtime_ms: number }>;
  maxFileBytes?: number;
...
export interface ScannedFile {
  info: IndexedFile;
  /** null for files read but unchanged from `previous`. */
  changed: boolean;
...
export interface ScanResult {
  root: string;
  files: ScannedFile[];
  /** Files whose disk state differs from `previous`. */
  changedPaths: string[];
  newPaths: string[];
  deletedPaths: string[];
...
export async function discoverFiles(
  ...
  async function walk(dir: string, rel: string, depth: number): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (matcher.ignores(childRel, entry.isDirectory())) continue;
      ...
      if (isBinaryishExtension(childRel)) continue;
      out.push({
        path: full,
        posix: childRel,
        size: st.size,
...
export function diffFiles(
  discovered: readonly { posix: string; mtime_ms: number; size: number }[],
  previous: ReadonlyMap<string, { hash: string; size: number; mtime_ms: number }>,
): { changed: string[]; newPaths: string[]; deleted: string[]; unchanged: string[] } {
  ...
  for (const path of previous.keys()) if (!seen.has(path)) deleted.push(path);
...
export async function scanProject(options: ScanOptions): Promise<ScanResult> {
  ...
  for (const file of discovered) {
    ...
    const verification: VerificationState = needsRead ? 'verified' : 'indexed';
    ...
    if (needsRead) changedPaths.push(file.posix);
  }
 ...
 */
export async function analyzeFile(path: string, info: IndexedFile): Promise<IndexedFile> {
...
export function resolveRelativeImports(
  ownerPath: string,
  imports: readonly { specifier: string; relative: boolean }[],
...
export function buildReverseDependencies(
  ...
  for (const [file, depends] of deps) {
    for (const target of depends) {
      ...
      else out.set(target, [file]);
    }
  }
  return out;
}

export function toRelPosix(root: string, path: string): string {
