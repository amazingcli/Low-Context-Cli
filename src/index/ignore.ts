...
export interface IgnoreRule {
  ...
  anchored: boolean;
  regex: RegExp;
  /** True when the pattern can only match a top-level path. */
}

export function compileIgnorePattern(pattern: string): IgnoreRule {
...
export class IgnoreMatcher {
  ...
  ignores(relativePath: string, isDirectory: boolean): boolean {
    ...
    for (const rule of this.rules) {
      ...
      if (rule.anchored && path.startsWith('.') && !path.startsWith(rule.raw.split('/')[0])) continue;
