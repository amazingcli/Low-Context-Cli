...
export interface OutputPolicy {
  /** Above this many characters, output is spilled to an artifact. */
  inline_limit: number;
  /** Bytes kept inline when spilling. */
  spill_head_lines: number;
  spill_tail_lines: number;
  /** Maximum characters of extracted error lines kept inline. */
  error_budget: number;
}
...
export interface ProcessedOutput {
  /** What the model sees. */
  inline: string;
  /** Where the full text was written, when it was too large. */
  artifactPath?: string;
  bytes: number;
  truncated: boolean;
  /** Error-ish lines found anywhere in the output. */
  errors: string[];
...
export class OutputManager {
  ...
  static extractErrors(text: string, budget = DEFAULT_OUTPUT_POLICY.error_budget): string[] {
    ...
    for (const line of text.split('\n')) {
      ...
      if (trimmed.length > 400) continue; // a wall of text is not a diagnostic
      if (!ERROR_PATTERNS.some((pattern) => pattern.re.test(trimmed))) continue;
      const key = trimmed.trim();
      if (seen.has(key)) continue;
      seen.add(key);
      if (used + key.length > budget) break;
      out.push(key);
      used += key.length + 1;
    }
    return out;
  ...
  async process(input: {
    text: string;
    label: string;
    /** Set when a failure produced no stdout at all. */
    error?: string;
    ...
    sections.push(
      `[output ${formatBytes(bytes)} — full copy at ${artifactPath}]`,
    );
    const head = headLines(text, this.policy.spill_head_lines);
    if (head.trim() !== '') sections.push(`--- first ${this.policy.spill_head_lines} lines ---\n${head}`);
    if (errors.length > 0) {
  ...
  async writeArtifact(label: string, text: string): Promise<string> {
