...
const rememberTool: ToolDefinition = {
  name: 'remember',
  description:
    'Store a durable memory record. Only use this for information the user explicitly asked to keep, or for a decision that was actually made in this conversation. Records are external storage, not model training.',
  category: 'memory',
  mutating: true,
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    if (!ctx.memoryStore) return { ok: false, summary: 'Memory storage is not available.', error: 'no memory store' };
    const summary = asString(args, 'summary').trim();
    if (summary === '') return { ok: false, summary: 'summary must not be empty', error: 'empty summary' };
    const type = (asOptionalString(args, 'type') as MemoryType | undefined) ?? 'FACT';
    if (!MEMORY_TYPES.includes(type)) return { ok: false, summary: `unknown memory type: ${type}`, error: 'bad type' };
    const importance = (asOptionalString(args, 'importance') as MemoryRecord['importance'] | undefined) ?? 'important';
    ...
    const record = await ctx.memoryStore.write({
      ...
      source_refs: [
        ...(sourceMessageId || ctx.sessionId
          ...
          : []),
        ...(filePath ? [{ kind: 'file' as const, file_path: filePath }] : []),
      ],
      ...(ctx.projectId === undefined ? {} : { project_id: ctx.projectId }),
      ...(ctx.sessionId === undefined ? {} : { session_id: ctx.sessionId }),
...
const searchMemoryTool: ToolDefinition = {
  name: 'search_memory',
  description:
    'Search stored memory records by keyword. Returns summaries with ids, confidence and importance. A memory summarises the past — verify against the source before acting on it.',
  category: 'memory',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    const type = asOptionalString(args, 'type') as MemoryType | undefined;
    ...
    if (hits.length === 0) return { summary: `No memory records match "${query}".`, verification_state: 'unknown' };
    const lines = hits.map(
      (hit, i) =>
        `${i + 1}. [${hit.record.id}] ${hit.record.type}/${hit.record.importance}/${hit.record.confidence} score=${hit.score.toFixed(2)}\n   ${hit.record.summary}\n   (${hit.reason}${hit.record.status !== 'current' ? `, ${hit.record.status}` : ''})`,
    );
    return {
...
const showMemoryTool: ToolDefinition = {
  name: 'show_memory',
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!record) return { ok: false, summary: 'No such memory record.', error: 'not found' };
    const lines = [
    ...
    if (record.superseded_by) lines.push(`superseded by: ${record.superseded_by}`);
...
const forgetTool: ToolDefinition = {
  name: 'forget',
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (hits.length === 0) return { summary: `No memory records matched "${query}"; nothing deleted.`, verification_state: 'unknown' };
    let deleted = 0;
...
const searchConversationsTool: ToolDefinition = {
  name: 'search_conversations',
  description:
    'Search historical conversation messages across sessions and projects. Use this for "what did we decide earlier" questions instead of relying on the current chat window.',
  category: 'memory',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    if (!ctx.conversationStore) return { ok: false, summary: 'Conversation storage is not available.', error: 'no conversation store' };
    const query = asString(args, 'query');
    ...
    if (hits.length === 0) return { summary: `No historical messages match "${query}".`, verification_state: 'unknown' };
...
const readMessagesTool: ToolDefinition = {
  name: 'read_messages',
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    const ids = asString(args, 'ids')
      ...
      .map((id) => id.trim())
      .filter((id) => id !== '');
    if (ids.length === 0) return { ok: false, summary: 'no message ids given', error: 'no ids' };
    const messages = await ctx.conversationStore.messagesByIds(ids);
    if (messages.length === 0) return { ok: false, summary: 'none of those message ids exist', error: 'not found' };
...
export async function applyMemoryDirective(
...
): Promise<{ handled: boolean; note?: string; cleaned: string }> {
  const control = parseMemoryControl(message);
  if (control.type === 'none') return { handled: false, cleaned: message };
  if (!ctx.memoryStore) return { handled: false, cleaned: message };
  ...
  if (control.type === 'forget') {
    ...
    if (target !== '') {
      ...
      for (const hit of hits) await ctx.memoryStore.delete(hit.record.id);
