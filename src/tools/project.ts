...
const gitDiffTool: ToolDefinition = {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (text.trim() === '') return { summary: 'No changes.', ok: true };
...
const gitLogTool: ToolDefinition = {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (commits.length === 0) return { summary: 'No matching commits.', ok: true };
    const summary = commits.map((c) => `${c.short} ${c.date.slice(0, 10)} ${c.author}: ${c.subject}`).join('\n');
    return { summary, verification_state: 'verified', data: { commits: commits as unknown as Record<string, unknown>[] } };
  },
};

/* -------------------------------- project_map ------------------------------ */

const projectMapTool: ToolDefinition = {
  name: 'project_map',
  description:
    'Show the compact project map: modules, directories and files with counts. Use it to navigate a project instead of listing every directory. Supply a module or path to narrow the view.',
  category: 'project',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    if (!ctx.indexStore || !ctx.projectId) return { ok: false, summary: 'No project index is available.', error: 'no index' };
    ...
    let scoped = files;
    if (wantedPath) scoped = scoped.filter((f) => f.path === wantedPath || f.path.startsWith(`${wantedPath.replace(/\/$/, '')}/`));
    if (wantedModule) scoped = scoped.filter((f) => f.path.startsWith(`${wantedModule}/`));
    if (scoped.length === 0) return { summary: 'No indexed files match that scope.', ok: true };
...
const findSymbolTool: ToolDefinition = {
  name: 'find_symbol',
  description:
    'Locate a function, class, method or type by name. Returns the file and line where it is defined, plus its verification state — the index may be stale, so verify before editing.',
  category: 'project',
  mutating: false,
  parameters: objectSchema(
    {
...
const findDependenciesTool: ToolDefinition = {
  name: 'find_dependencies',
  description:
    'Show what a file imports and what imports it, from the project dependency graph. Use it to find the blast radius of a change.',
  category: 'project',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!file) return { ok: false, summary: `Not in the index: ${path}`, error: 'not indexed' };
    const lines: string[] = [`${file.path} [${file.verification_state}${file.stale ? ', stale' : ''}]`];
...
const verifySourceTool: ToolDefinition = {
  name: 'verify_source',
  description:
    'Read the real source of a symbol or file region from disk and compare it with what the index claims. Use this before any important edit: summaries and indexes can be stale.',
  category: 'project',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!isInside(ctx.projectRoot, abs)) return { ok: false, summary: `path escapes the project: ${path}`, error: 'outside project' };
    let text: string;
    try {
...
const describeFileTool: ToolDefinition = {
  name: 'describe_file',
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!file) return { ok: false, summary: 'File is not in the index.', error: 'not indexed' };
...
const indexStatusTool: ToolDefinition = {
  ...
  async run(_args, ctx): Promise<ToolRunResult> {
    ...
    for (const file of files) {
      ...
      bytes += file.size;
      if (file.stale || file.verification_state === 'stale') stale += 1;
    }
...
export function renderScope(tree: Parameters<typeof renderTree>[0], depth: number, maxFiles: number): string {
