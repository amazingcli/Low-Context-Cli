...
import type { ToolDefinition, ToolContext, ToolRunResult } from './types.js';
...
const listDirectory: ToolDefinition = {
  name: 'list_directory',
  description:
    'List the entries of a directory inside the project. Directories are marked with a trailing slash. Use this to confirm structure before reading files.',
  category: 'filesystem',
  mutating: false,
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!resolved.ok) return { ok: false, summary: resolved.reason, error: resolved.reason };
    const lines: string[] = [];
    let count = 0;
    ...
    async function walk(dir: string, rel: string, maxDepth: number, level: number, out: string[], context: ToolContext, bump: () => void): Promise<void> {
      if (level >= maxDepth) return;
      let entries;
      try {
        entries = await readdir(dir, { withFileTypes: true });
      } catch (error) {
        out.push(`${'  '.repeat(level)}[unreadable: ${(error as Error).message}]`);
        return;
      }
      entries.sort((a, b) => Number(b.isDirectory()) - Number(a.isDirectory()) || a.name.localeCompare(b.name));
      for (const entry of entries) {
        if (entry.name === '.git' || entry.name === 'node_modules' || entry.name === '.low-context') continue;
        bump();
...
const readFileTool: ToolDefinition = {
  name: 'read_file',
  description:
    'Read a file from the project as line-numbered text. For large files, pass offset and limit to read a window instead of the whole file. Returns the actual current source — always prefer this over trusting an index summary.',
  category: 'filesystem',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!info) return { ok: false, summary: `file not found: ${resolved.rel}`, error: `file not found: ${resolved.rel}` };
    if (info.isDirectory()) return { ok: false, summary: `${resolved.rel} is a directory; use list_directory`, error: 'is a directory' };

    const text = await tryRead(resolved.abs);
    if (text === undefined) return { ok: false, summary: `could not read ${resolved.rel} (binary or unreadable)`, error: 'unreadable' };
...
const writeFileTool: ToolDefinition = {
  name: 'write_file',
  description:
    'Create or overwrite a file with the given content. Overwrites destroy the previous contents, so prefer edit_file for changes to existing files.',
  category: 'filesystem',
  mutating: true,
  parameters: objectSchema(
    {
...
const editFileTool: ToolDefinition = {
  name: 'edit_file',
  description:
    'Replace an exact string in an existing file. The old_string must appear verbatim, including indentation, and must be unique unless replace_all is true. This is the safe way to modify code: it cannot silently clobber a region the model did not look at.',
  category: 'filesystem',
  mutating: true,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (original === undefined) return { ok: false, summary: `file not found: ${resolved.rel}`, error: 'file not found' };
    if (oldString === '') return { ok: false, summary: 'old_string must not be empty', error: 'empty old_string' };
    if (oldString === newString) return { ok: false, summary: 'old_string and new_string are identical; nothing to do', error: 'no-op edit' };
...
const deleteFileTool: ToolDefinition = {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!resolved.ok) return { ok: false, summary: resolved.reason, error: resolved.reason };
    const info = await statSafe(resolved.abs);
    if (!info) return { ok: false, summary: `file not found: ${resolved.rel}`, error: 'file not found' };
    if (info.isDirectory()) return { ok: false, summary: `refused: ${resolved.rel} is a directory`, error: 'is a directory' };
...
const moveFileTool: ToolDefinition = {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (!from.ok) return { ok: false, summary: from.reason, error: from.reason };
    if (!to.ok) return { ok: false, summary: to.reason, error: to.reason };
...
const searchTextTool: ToolDefinition = {
  name: 'search_text',
  description:
    'Search project files for a literal string or regular expression and return matching lines with file and line numbers. Prefer this over reading many files when you only need to locate something.',
  category: 'filesystem',
  mutating: false,
  parameters: objectSchema(
    {
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    try {
      ...
      return { ok: false, summary: `invalid regular expression: ${(error as Error).message}`, error: 'invalid regex' };
    }

    const scope = resolveProjectPath(ctx, asOptionalString(args, 'path') ?? '.');
    if (!scope.ok) return { ok: false, summary: scope.reason, error: scope.reason };
    const scopeInfo = await statSafe(scope.abs);
    if (!scopeInfo) return { ok: false, summary: `path not found: ${scope.rel}`, error: 'path not found' };
    ...
    for (const candidate of candidates) {
      ...
      if (candidate.rel.includes('node_modules/') || candidate.rel.startsWith('.git/')) continue;
...
export async function outlineFile(path: string): Promise<string> {
  const text = await tryRead(path);
  if (text === undefined) return '';
  const lines = text.split('\n');
  const total = lines.length;
  if (total <= 200) return text;
