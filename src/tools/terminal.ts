...
export function executeCommand(command: string, options: ExecuteOptions): Promise<CommandOutcome> {
  return new Promise((resolve) => {
    ...
    const append = (chunk: string, stream: 'stdout' | 'stderr'): void => {
      ...
      if (target.length >= options.maxOutputBytes) {
        ...
        if (stream === 'stdout') stdout = cut;
        else stderr = cut;
      } else if (stream === 'stdout') stdout = next;
      else stderr = next;
...
const runCommandTool: ToolDefinition = {
  name: 'run_command',
  description:
    'Run a shell command in the project root and return its exit code with stdout and stderr. Use it to build, test, inspect git state or reproduce a failure. Long output is truncated with the full text saved to an artifact.',
  category: 'terminal',
  mutating: true,
  ...
  async run(args, ctx): Promise<ToolRunResult> {
    ...
    if (command.trim() === '') return { ok: false, summary: 'empty command', error: 'empty command' };
    if (ctx.signal?.aborted) return { ok: false, summary: 'cancelled before execution', error: 'cancelled' };
    ...
    const outcome = await executeCommand(command, {
      ...
      signal: ctx.signal,
      onData: (chunk, stream) => {
        if (ctx.config.ui.response_mode === 'verbose' && stream === 'stderr') ctx.note?.(chunk.trimEnd());
      },
    });
...
export const TERMINAL_TOOLS: ToolDefinition[] = [runCommandTool];
