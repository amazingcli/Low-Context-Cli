...
import type { ChatMessage, Conversation } from '../core/types.js';
import type {
  AppendMessageInput,
  ConversationQuery,
  ConversationStore,
...
export interface ConversationStoreOptions {
  /** Base directory that holds `conversations/`. */
  baseDir: string;
}

export class FileConversationStore implements ConversationStore {
  private readonly dir: string;
  private readonly logs = new Map<string, JsonlLog<ChatMessage>>();
  private readonly metas = new Map<string, Conversation>();
  private readonly index: PersistentLexicalIndex;
  private catalog: Map<string, CatalogEntry> | undefined;
  private catalogLog: JsonlLog<CatalogEntry & { id: string }> | undefined;
  private indexDirtyIds = new Set<string>();
  ...
  private async log(id: string): Promise<JsonlLog<ChatMessage>> {
    let existing = this.logs.get(id);
    if (!existing) {
      existing = new JsonlLog<ChatMessage>(this.logPath(id));
      await existing.open();
  ...
  private async loadCatalog(): Promise<Map<string, CatalogEntry>> {
    ...
    const log = new JsonlLog<CatalogEntry & { id: string }>(path, { flushEvery: 256 });
    await log.open();
    this.catalogLog = log;
    const map = new Map<string, CatalogEntry>();
    for await (const entry of log.iterate()) map.set(entry.id, entry);
    this.catalog = map;
    return map;
  }

  async create(input: { project_id?: string; session_id?: string; title?: string } = {}): Promise<Conversation> {
    await this.ensure();
    const now = nowIso();
  ...
  async get(id: string): Promise<Conversation | undefined> {
    ...
    const meta = await readJsonIfExists<Conversation>(this.metaPath(id));
    if (meta) this.metas.set(id, meta);
    return meta;
  }

  async list(input: { project_id?: string; limit?: number } = {}): Promise<Conversation[]> {
    await this.ensure();
    ...
    for (const name of entries) {
      ...
      if (!meta) continue;
      if (input.project_id && meta.project_id !== input.project_id) continue;
      out.push(meta);
    }
  ...
  async append(conversationId: string, input: AppendMessageInput): Promise<ChatMessage> {
    const [message] = await this.appendMany(conversationId, [input]);
    return message as ChatMessage;
  }

  async appendMany(conversationId: string, inputs: AppendMessageInput[]): Promise<ChatMessage[]> {
    await this.ensure();
  ...
  async messages(conversationId: string, options: { limit?: number; offset?: number } = {}): Promise<ChatMessage[]> {
    const log = await this.log(conversationId);
  ...
  async recentMessages(conversationId: string, count: number): Promise<ChatMessage[]> {
    const log = await this.log(conversationId);
  ...
  async searchMessages(
  ...
  ): Promise<{ message: ChatMessage; score: number; reason: string }[]> {
    ...
    const catalog = await this.loadCatalog();
    const results: { message: ChatMessage; score: number; reason: string }[] = [];
    const metaCache = new Map<string, ChatMessage>();

    for (const hit of hits) {
      const entry = catalog.get(hit.id);
      if (!entry) continue;
      if (query.conversation_id && entry.conversation_id !== query.conversation_id) continue;
      ...
      if (query.roles && !query.roles.includes(message.role)) continue;
      if (query.session_id && message.session_id !== query.session_id) continue;
      if (query.since && message.timestamp < query.since) continue;
      if (query.project_id) {
        const conversation = await this.get(entry.conversation_id);
        if (conversation?.project_id !== query.project_id) continue;
      }
      results.push({
        message,
        score: hit.score,
  ...
  async messagesByIds(ids: readonly string[]): Promise<ChatMessage[]> {
  ...
  async updateSummary(conversationId: string, summary: string, throughMessageId: string): Promise<void> {
  ...
  async delete(conversationId: string): Promise<boolean> {
    ...
    for (const id of gone) catalog.delete(id);
  ...
  async retain(conversationId: string, keep: (message: ChatMessage) => boolean): Promise<number> {
    const log = await this.log(conversationId);
    const removed: string[] = [];
    const { removed: removedCount } = await log.retain((message) => {
      const verdict = keep(message);
      if (!verdict) removed.push(message.id);
      return verdict;
    });
  ...
  async flush(): Promise<void> {
    for (const log of this.logs.values()) await log.flush();
    await this.catalogLog?.flush();
    await this.index.flush();
  }

  /** Diagnostics for `doctor`. */
  async stats(): Promise<{ conversations: number; messages: number; indexed_docs: number; catalog_entries: number }> {
    const conversations = await this.list();
    const catalog = await this.loadCatalog();
    return {
  ...
  async rebuild(): Promise<{ conversations: number; messages: number; duration_ms: number }> {
...
export function isMessageId(value: string): boolean {
  return safeJsonParse<string>(JSON.stringify(value)).ok && value.startsWith('msg_');
}
