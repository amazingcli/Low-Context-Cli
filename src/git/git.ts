...
async function git(cwd: string, args: string[], maxBuffer = 4 * 1024 * 1024): Promise<GitRun> {
  try {
...
export async function changedFiles(cwd: string): Promise<GitChangedFiles> {
  const result = await git(cwd, ['status', '--porcelain=v1']);
  const out: GitChangedFiles = { added: [], modified: [], deleted: [], renamed: [], untracked: [] };
  if (!result.ok) return out;
  for (const line of result.stdout.split('\n')) {
    if (line.trim() === '') continue;
    const code = line.slice(0, 2);
    const path = line.slice(3).trim();
    const target = path.includes(' -> ') ? (path.split(' -> ')[1] as string) : path;
    if (code === '??') out.untracked.push(target);
    else if (code.includes('R')) out.renamed.push(target);
    else if (code.includes('D')) out.deleted.push(target);
    else if (code.includes('A')) out.added.push(target);
    else out.modified.push(target);
  }
  return out;
}

export async function diff(cwd: string, options: { staged?: boolean; paths?: string[]; context?: number } = {}): Promise<string> {
...
export async function log(cwd: string, options: { limit?: number; paths?: string[]; grep?: string } = {}): Promise<GitCommit[]> {
  const args = ['log', `-n${options.limit ?? 20}`, '--pretty=format:%H%x1f%h%x1f%an%x1f%aI%x1f%s%x1e'];
  if (options.grep) args.push(`--grep=${options.grep}`, '-i');
  if (options.paths && options.paths.length > 0) args.push('--', ...options.paths);
  const result = await git(cwd, args);
  if (!result.ok) return [];
...
export async function blame(cwd: string, path: string, maxLines = 200): Promise<{ line: number; commit: string; author: string; text: string }[]> {
  const result = await git(cwd, ['blame', '--line-porcelain', '--', path]);
  if (!result.ok) return [];
  const out: { line: number; commit: string; author: string; text: string }[] = [];
  let commit = '';
  let author = '';
  let line = 1;
...
export async function changedPathsForRetrieval(cwd: string): Promise<{ added: string[]; modified: string[]; deleted: string[] }> {
...
export async function show(cwd: string, ref: string, maxBytes = 32_768): Promise<string> {
  const result = await git(cwd, ['show', '--stat', '--no-color', ref]);
  if (!result.ok) return '';
  return result.stdout.slice(0, maxBytes);
}

export async function shortStatusSummary(cwd: string): Promise<string> {
