/**
 * Memory engine facade (§35, §48, §65).
 *
 * This is the *only* place outside the CLI layer that decides *what becomes
 * memory*. The rule: memory is learned through verified events —
 *
 *   - explicit user statements ("remember this", "don't forget we use X")
 *   - confirmed tool results (a test run exit code, a successfully applied edit)
 *   - confirmed decisions and successful operations
 *
 * Model-generated guesses are never promoted to durable facts automatically.
 * Every write records provenance (`source_refs`), confidence, and importance.
 */
import type { AgentEvent } from '../core/types.js';
import type { MemoryType, MemoryRecord, SourceReference } from '../core/types.js';
...
export interface MemoryExtractionResult {
  /** Candidates ready to hand to `MemoryStore.write` — ids and timestamps are the store's job. */
  written: MemoryWriteInput[];
  skipped: { summary: string; reason: string }[];
}

/** Signals that make a user message worth remembering (§10). */
const USER_MEMORY_SIGNALS: { re: RegExp; type: MemoryType }[] = [
...
export function candidatesFromUserMessage(message: string, context: MemoryAgentContext): MemoryWriteInput[] {
...
export function candidateFromExplicitRemember(message: string, context: MemoryAgentContext): MemoryWriteInput | undefined {
  const match = /^(?:please\s+)?(?:remember|memorize|note(?: down)?)\s*(?:this|that)?\s*:?\s*(.+)$/i.exec(message.trim());
  if (!match) return undefined;
  const summary = (match[1] as string).trim();
...
/** Whether a directive tells the agent to forget something (§65). */
export interface MemoryControl {
  type: 'remember' | 'forget' | 'dont_remember' | 'show' | 'none';
  target?: string;
}

export function parseMemoryControl(message: string): MemoryControl {
  const trimmed = message.trim();
  if (/^\s*(?:please\s+)?(?:remember|memorize|note down)\b/i.test(trimmed)) return { type: 'remember', target: trimmed };
  ...
  if (/don'?t remember|do not remember|don'?t store|do not store|don'?t save|do not save/i.test(trimmed)) return { type: 'dont_remember' };
  if (/^\s*(?:show|list|what|display)\s+(?:my )?memory/i.test(trimmed)) return { type: 'show' };
  return { type: 'none' };
}

/** Message content stripped of an explicit memory-control directive. */
export function stripMemoryControl(message: string, control: MemoryControl): string {
  if (control.type === 'dont_remember') return message;
  if (control.type === 'remember') return message.replace(/^\s*(?:please\s+)?(?:remember|memorize|note down)\s*(?:this|that)?\s*:?\s*/i, '');
  return message;
}
...
export function extractFromEvents(
  ...
  minImportance: 'temporary' | 'normal' = 'normal',
): MemoryExtractionResult {
  const written: MemoryWriteInput[] = [];
...
function describeVerificationEvent(data: Record<string, unknown>): string {
  if (typeof data.summary === 'string' && data.summary !== '') return data.summary;
  if (typeof data.command === 'string' && data.command !== '') return data.command;
  return 'Verification run';
}

/** Raise a record's importance to the configured floor, never lower it. */
function atLeast(floor: 'temporary' | 'normal'): MemoryRecord['importance'] {
  const normal: MemoryRecord['importance'] = 'normal';
...
export function correctionWrite(
  oldId: string,
  summary: string,
  type: MemoryType,
  source: SourceReference,
  context: MemoryAgentContext,
  confidence: 'verified' | 'high' = 'verified',
): MemoryWriteInput {
  return {
    type,
    scope: context.projectId ? 'project' : 'global',
...
export function defaultConfigImportance(config: LowContextConfig): 'temporary' | 'normal' {
