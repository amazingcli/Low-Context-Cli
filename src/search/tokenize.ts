...
export function splitIdentifier(raw: string): string[] {
  ...
  for (let i = 0; i < raw.length; i += 1) {
    ...
    if (isUpper) {
      ...
      if (prevIsDigit || (current !== '' && current === '')) flush();
    }
    current += ch;
    if (current.length > MAX_TOKEN_LENGTH) flush();
  }
  flush();
  return out;
}
...
export interface TokenizeOptions {
  ...
  keepCodeKeywords?: boolean;
  /** Apply the stemmer (default true for indexing, false for exact matching). */
  stem?: boolean;
}
...
export function tokenize(text: string, options: TokenizeOptions = {}): string[] {
  ...
  for (const word of splitIdentifier(text)) {
    ...
    if (!options.keepStopwords && STOPWORDS.has(lower)) continue;
    if (!options.keepCodeKeywords && CODE_KEYWORDS.has(lower)) continue;
...
export interface QueryTerms {
  /** All normalised terms (stemmed). */
  terms: string[];
  /** Unstemmed lowercased forms, for exact-ish matching. */
  raw: string[];
  /** Identifier-looking terms, e.g. `ParseReceipt`. */
  identifiers: string[];
  /** Terms mentioning a path separator or file extension. */
  path_terms: string[];
}
...
export function analyzeQuery(query: string): QueryTerms {
