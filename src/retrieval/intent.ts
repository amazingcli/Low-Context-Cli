/**
 * Query understanding (§61).
 *
 * Retrieval must start from *intent*, not the literal string. "Why is the
 * payment system slow?" should retrieve the payment module, its database
 * queries, caching, logs and recent payment decisions — not just the words
 * "payment slow".
 *
 * Classification here is heuristic (no model round-trip on every retrieval).
 * It extracts:
 *  - kind (code change / question / analysis / memory / chat)
 *  - paths and symbols named by the user
 *  - likely subsystems (modules) from a project map
 *  - historical intent ("what did we decide last month")
 *  - whole-project intent ("analyze the entire app")
 */
import type { IntentKind, QueryIntent, IndexedModule } from '../core/types.js';

const WHOLE_PROJECT_RE =
  /\b(whole|entire|full|all)\s+(app|application|project|codebase|repository|repo|code)\b|\banalyze\s+(the|this)?\s*(app|whole|entire|project)\b/;

const HISTORICAL_RE =
  /\b(last|previous|earlier|yesterday|last month|last week|before|back when|a while ago|we (decided|discussed|talked|added|removed|changed))\b/;
...
const CHANGE_RE =
  /\b(fix|fixes|fixing|change|update|add|remove|delete|create|refactor|implement|rewrite|repair|migrate|move|rename|improve|optimize|debug|break|edit|patch)\b/;

/** Words that describe a defect without naming a fix action. */
const PROBLEM_RE =
  /\b(broken|break[sz]?|failing|fails?|failure|wrong|incorrect|mishandl\w*|bug|bugs|crash\w*|error|errors|slow|sluggish|hang\w*|timeout|timeouts|stuck|leak\w*|regression|not working|doesn'?t work|isn'?t working|throws?)\b/i;
 ...
 */
const MEMORY_RE =
  /\b(remember\w*|memor(?:y|ies)|earlier|previously|last (?:time|month|week|year)|we (?:decided|chose|agreed|discussed|talked)|what did we|history)\b/i;

export function understandQuery(raw: string, modules: readonly IndexedModule[] = []): QueryIntent {
  const text = raw.trim();
  ...
  let kind: IntentKind = 'chat';
  if (wholeProject) kind = 'analysis';
  else if (isMemoryY) kind = 'memory_query';
  else if (isChange || problemWithCode) kind = 'code_change';
...
/** Map an intent to a retrieval plan (stage list with limits). */
export interface RetrievalStagePlan {
  stages: { name: string; limit: number }[];
  priorityModule?: string;
}

export function planForIntent(intent: QueryIntent): RetrievalStagePlan {
