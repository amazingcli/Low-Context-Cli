...
import { loadConfig, type LowContextConfig } from '../core/config.js';
import { AgentEventLog, Logger } from '../core/logger.js';
import { describeError, isLowContextError } from '../core/errors.js';
import { openWorkspace, type Workspace } from '../agent/workspace.js';
import { COMMANDS, HELP, VERSION, type CommandContext } from './commands.js';
...
export interface MainOptions {
  /** Override for tests: the project directory to operate on. */
  cwd?: string;
  argv?: string[];
}
...
export async function main(options: MainOptions = {}): Promise<number> {
  ...
  const ctx: CommandContext = {
    ...
    async openWorkspace(openOptions = {}) {
      if (cachedWorkspace) {
        if (openOptions.refresh === true) await cachedWorkspace.ensureIndex({ force: false });
  ...
  try {
    ...
    await logger.flush();
    if (cachedWorkspace) await cachedWorkspace.flush();
    return code;
  } catch (error) {
    ...
    if (isLowContextError(error)) {
      ...
      if (error.fix) ui.out(ui.dim(`  fix: ${error.fix}`));
    } else {
      ui.error(describeError(error));
      if (config.ui.response_mode === 'debug' && error instanceof Error && error.stack) ui.out(ui.dim(error.stack));
    }
    return 1;
  }
}
...
async function dispatch(
  ...
  hooks: { offerSetup: () => Promise<boolean> },
): Promise<number> {
  ...
  if (command === 'run' || command === 'ask') {
    ...
    if (!(await ensureModel(ctx, hooks))) return 2;
...
async function ensureModel(ctx: CommandContext, hooks: { offerSetup: () => Promise<boolean> }): Promise<boolean> {
...
function applyFlagOverrides(config: LowContextConfig, parsed: ReturnType<typeof parseArgs>): LowContextConfig {
  const next: LowContextConfig = { ...config };
  ...
  if (flagBool(parsed.flags, 'quiet')) next.ui = { ...next.ui, response_mode: 'quiet' };
  if (flagBool(parsed.flags, 'verbose')) next.ui = { ...next.ui, response_mode: 'verbose' };
  if (flagBool(parsed.flags, 'debug')) next.ui = { ...next.ui, response_mode: 'debug' };
  ...
  if (provider !== undefined) next.active_provider = provider;
  const model = flagString(parsed.flags, 'model');
  if (model !== undefined) next.active_model = model;
  if (flagBool(parsed.flags, 'no-memory')) next.memory = { ...next.memory, enabled: false, auto_capture: false };
  if (flagBool(parsed.flags, 'no-index')) next.index = { ...next.index, enabled: false, refresh_on_start: false };
  return next;
}
