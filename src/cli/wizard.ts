...
interface Preset {
  ...
  label: string;
  detail: string;
  /** Default base URL; also the value offered when the wizard asks for one. */
  baseUrl?: string;
  env: string;
  keyRequired: boolean;
  /** Ask for a base URL instead of using the default silently. */
  askBaseUrl: boolean;
  /** The `/models` endpoint makes sense for this provider. */
...
export async function runInitWizard(ctx: CommandContext, options: { nonInteractive?: boolean } = {}): Promise<number> {
  const { ui } = ctx;
  ui.out('');
  ...
  if (testNow !== 'n' && testNow !== 'no') await testConnection(ui, provider, apiKey);
  ...
  if (indexNow) {
    ...
    ui.stopSpinner();
    if (result) ui.success(`Indexed ${result.added + result.changed + result.unchanged} file(s) in ${result.duration_ms} ms.`);
    else ui.warn('Indexing is disabled in config.');
  }
...
async function pickProvider(ui: Ui): Promise<Preset> {
  ...
  return PRESETS[index] as Preset;
}

async function pickBaseUrl(ui: Ui, preset: Preset): Promise<string | undefined> {
  if (!preset.askBaseUrl) return preset.baseUrl;
  ui.out('');
...
async function collectKey(ui: Ui, preset: Preset): Promise<string | undefined> {
  if (preset.env === '') return undefined;
  ...
  if (envKey !== undefined && envKey !== '') {
    ...
    if (!useIt.startsWith('p')) return undefined; // env var wins; nothing to store
  ...
  if (!answer.startsWith('b')) return undefined;
  const key = await ui.askSecret(`Paste the ${preset.label} API key (input hidden)`);
  if (key === '') ui.warn('No key entered; continuing without one.');
...
async function pickModel(
  ui: Ui,
  preset: Preset,
  settings: { baseUrl: string | undefined; apiKey: string | undefined },
): Promise<PresetModel> {
  for (;;) {
    ui.out('');
    ...
    if (preset.canListModels) ui.out(`  ${fetchIndex}) ${ui.bold('Fetch the model list from this provider')} ${ui.dim('— uses /models')}`);
    ...
    if (Number.isFinite(value) && value >= 1 && value <= preset.models.length) {
      return preset.models[value - 1] as PresetModel;
    }
    ui.warn('Pick one of the listed numbers.');
  }
}

async function pickFromList(ui: Ui, models: PresetModel[]): Promise<PresetModel | undefined> {
  ...
  if (models.length > shown.length) ui.out(ui.dim(`  …and ${models.length - shown.length} more (paste one with the manual option)`));
  ui.out(`  0) back`);
  const answer = (await ui.ask('Enter a number', '0')).trim();
  const value = Number(answer);
  if (!Number.isFinite(value) || value < 1 || value > shown.length) return undefined;
  return shown[value - 1] as PresetModel;
}
...
async function testConnection(ui: Ui, provider: ProviderConfig, apiKey: string | undefined): Promise<void> {
  ...
  try {
    ...
    if (models.length > 0) ui.success(`Connection OK — ${models.length} model(s) reachable.`);
    else ui.warn('The provider responded but listed no models. This is normal for some gateways; if requests fail, check the model ID with /models.');
  } catch (error) {
 ...
 */
async function loadBaseConfig(): Promise<LowContextConfig> {
...
export async function offerSetup(ui: Ui, ctx: CommandContext): Promise<boolean> {
