 ...
 */
import { ensureGlobalConfig, saveGlobalConfig, type LowContextConfig, type ProviderConfig, type ProviderKind } from '../core/config.js';
...
interface Preset {
  ...
  baseUrl?: string;
  env: string;
  models: { id: string; label: string; context_limit?: number }[];
  needsKey: boolean;
}
...
export async function runInitWizard(ctx: CommandContext, options: { nonInteractive?: boolean } = {}): Promise<number> {
  ...
  const preset = PRESETS[Number.isFinite(choice) ? Math.max(0, Math.min(PRESETS.length - 1, choice - 1)) : 0] as Preset;
  ...
  const model = preset.models[Number.isFinite(modelChoice) ? Math.max(0, Math.min(preset.models.length - 1, modelChoice - 1)) : 0] as Preset['models'][number];
