...
import { registryFromConfig, type ToolRegistry } from '../tools/registry.js';
...
import { SLASH_COMMANDS, type MenuColors } from './slash-menu.js';
...
/** Mutable session state so slash commands can change the model mid-session. */
interface SessionRuntime {
  ...
  conversationId: string;
  taskState?: TaskState;
  /** Session-scoped permission bypass, toggled by /nopermission. */
  bypass: boolean;
  rebuild(): void;
}
...
async function handleSlashCommand(
  ...
  line: string,
  totals: { input: number; output: number; turns: number; tools: number; estimated: boolean },
  ...
  switch (command) {
    ...
    case 'history': {
      ...
      for (const message of messages) ui.out(formatHistoryLine(ctx, message));
      ui.out('');
    ...
    case 'sessions': {
      ...
      ui.table(
        ...
        sessions.map((s) => [s.id, s.id === runtime.session.id ? `${s.status} (current)` : s.status, `${s.provider ?? '?'}/${s.model ?? '?'}`, s.updated_at.slice(0, 19)]),
      );
      return {};
    }
    case 'resume': {
      ...
      if (target.conversation_id) runtime.conversationId = target.conversation_id;
      ...
      if (runtime.taskState) ui.out(renderTaskState(runtime.taskState));
      return {};
    }
    case 'clear': {
      const conversation = await workspace.stores.conversations.create({
    ...
    /* --------------------------------- model -------------------------------- */
    case 'model': {
      if (argument !== '') return switchModel(ctx, runtime, argument);
    ...
    case 'doctor': {
      ...
      if (doctor) await doctor(ctx, argument);
      else ui.warn('The doctor command is unavailable in this build.');
      return {};
    }
    default:
...
async function switchModel(ctx: CommandContext, runtime: SessionRuntime, argument: string): Promise<SlashOutcome> {
  const { ui } = ctx;
  const models = await listConfiguredModels(ctx);
...
interface ModelChoice {
  provider: string;
  providerConfig: Parameters<typeof createProvider>[0];
  model: ModelDescriptor;
}

async function listConfiguredModels(ctx: CommandContext): Promise<ModelChoice[]> {
  const out: ModelChoice[] = [];
  for (const provider of ctx.config.providers) {
    if (!provider.enabled || provider.kind === 'mock') continue;
  ...
  if (out.length === 0) {
    for (const provider of ctx.config.providers) {
      for (const model of provider.models) out.push({ provider: provider.id, providerConfig: provider, model: modelDescriptor(provider, model) });
    }
  }
  return out;
}

async function describeCredential(ctx: CommandContext, providerId: string, envName: string | undefined): Promise<string> {
  if (envName && process.env[envName]) return `environment variable ${envName}`;
  ...
  if (stored) return 'stored in credentials.json (0600)';
  if (envName) return ctx.ui.color('yellow', `not set (expected ${envName}, or store it with \`lc init\`)`);
  return ctx.ui.dim('none');
}

function formatHistoryLine(ctx: CommandContext, message: ChatMessage): string {
  const { ui } = ctx;
...
function renderSlashHelp(): string {
  ...
  const lines: string[] = ['Commands (the same list appears when you type "/")'];
...
async function projectCounts(workspace: Workspace): Promise<{ files: number; memory: number }> {
