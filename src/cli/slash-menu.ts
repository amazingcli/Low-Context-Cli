...
export interface SlashCommandSpec {
  ...
  name: string;
  /** Argument hint shown in the menu; presence means "Enter fills the line". */
  args?: string;
  description: string;
  /** Group headers keep the list scannable (§28: readable terminal UI). */
  group: 'session' | 'project' | 'memory' | 'model' | 'system';
}

/** Score how well `query` matches a command; lower is better, ∞ = no match. */
export function matchScore(spec: SlashCommandSpec, query: string): number {
  if (query === '') return 0;
  ...
  let i = 0;
  for (const char of name) if (char === q[i]) i += 1;
  if (i === q.length) return 3;
  if (spec.description.toLowerCase().includes(q)) return 4;
  return Number.POSITIVE_INFINITY;
}

export function filterCommands(specs: readonly SlashCommandSpec[], query: string, limit = 8): SlashCommandSpec[] {
  return specs
...
export function renderMenuLines(
  filtered: readonly SlashCommandSpec[],
  query: string,
  selected: number,
  colors: MenuColors,
  width = 72,
  ...
  for (let i = 0; i < filtered.length; i += 1) {
    const spec = filtered[i] as SlashCommandSpec;
