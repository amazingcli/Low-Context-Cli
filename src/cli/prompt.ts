...
export interface ReadLineOptions {
  ...
  history?: string[];
  /** Menu rows to show at once. */
  maxMenuRows?: number;
}

export interface ReadLineResult {
  /** `undefined` means the user cancelled (Ctrl-C / Ctrl-D). */
  line: string | undefined;
  /** How the line ended, so callers can distinguish cancel from end. */
  ended: 'submitted' | 'eof' | 'interrupted';
}
...
const KEY = {
  up: (key: string) => key === `${ESC}A` || key === `${ESC}OA`,
  down: (key: string) => key === `${ESC}B` || key === `${ESC}OB`,
  right: (key: string) => key === `${ESC}C` || key === `${ESC}OC`,
  left: (key: string) => key === `${ESC}D` || key === `${ESC}OD`,
  home: (key: string) => key === `${ESC}H` || key === `${ESC}1~` || key === `${ESC}OH`,
  end: (key: string) => key === `${ESC}F` || key === `${ESC}4~` || key === `${ESC}OF`,
  delete: (key: string) => key === `${ESC}3~`,
};

function isPrintable(key: string): boolean {
  if (key.length === 0) return false;
  const code = key.codePointAt(0) as number;
...
export async function readLine(options: ReadLineOptions): Promise<ReadLineResult> {
  if (stdin.isTTY !== true) return plainReadLine(options.prompt);
  try {
 ...
 */
const pipedQueue: string[] = [];
const pipedWaiters: ((line: string | undefined) => void)[] = [];
let pipedDone = false;
let pipedStarted = false;

function startPiped(): void {
  const reader = createInterface({ input: stdin });
  reader.on('line', (line: string) => {
    const waiter = pipedWaiters.shift();
    if (waiter) waiter(line);
    else pipedQueue.push(line);
  });
  reader.on('close', () => {
    pipedDone = true;
    for (const waiter of pipedWaiters.splice(0)) waiter(undefined);
  });
}

async function plainReadLine(prompt: string): Promise<ReadLineResult> {
  if (stdin.isTTY === true) {
  ...
  if (queued !== undefined) return { line: queued.trim(), ended: 'submitted' };
  if (pipedDone) return { line: undefined, ended: 'eof' };
  const answer = await new Promise<string | undefined>((resolve) => pipedWaiters.push(resolve));
  if (answer === undefined) return { line: undefined, ended: 'eof' };
  return { line: answer.trim(), ended: 'submitted' };
}

async function rawReadLine(options: ReadLineOptions): Promise<ReadLineResult> {
  ...
  return await new Promise<ReadLineResult>((resolve) => {
    ...
    const cancel = (ended: 'eof' | 'interrupted') => {
      erase();
      out('\n');
    ...
    const onData = (chunk: string | Buffer) => {
      ...
      for (const key of parseKeys(text)) {
        ...
        if (key === '\t') {
          const spec = filtered[selected];
          if (menuOpen && spec) replaceBuffer(`/${spec.name} `);
          continue;
        }
        if (key === '\u001b') {
          if (menuOpen) {
        ...
        if (KEY.down(key)) {
          ...
          if (historyIndex >= history.length) continue;
